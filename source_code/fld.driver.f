...
C  Distributed under the GNU General Public License, version 3
     ...
     e                MXEFV, MAPEFV, NEFV, NEFVP, SCALAM, SVNAME, SVUNIT
      USE potential, ONLY: CONLEN, EPNAME, IREF, NCONST, NDGVL, NEXTRA,
     p                     NRSQ, NVLBLK, RMNAME
      USE basis_data, ONLY: NJLQN9
      USE physical_constants, ONLY: NIST_year, bfct
      USE sizes, ONLY: MXNRG => MXNRG_in_FIELD, MXNODE, MXOMEG
C
C***********************************************************************
C
...
C
     ...
     1                 BCYOMX, CONFRQ, CONLEN, DEGTOL, DFIELD,
     2                 DNRG,   DR,     DRAIRY, DRL,    DRS,
     3                 DTOL,   ENERGY, EPL,    EPS,    EREF,
     4                 EUNAME, EUNITS, EUNIT,  FIELD,  FIXFLD,
     5                 FLDMAX, FLDMIN, IBDSUM, IBFIX,  IBHI,
     6                 IFIELD, IFVARY, IMGSEL, INTFLG, IPRINT,
     7                 IPROPL, IPROPS, IPSISC, IREF,   IRMSET,
     8                 ISCRU,  IWAVE,  IWAVEF, IWAVSC, IWVSTP,
     9                 JSTEP,  JTOTL,  JTOTU,  KSAVE,  LABEL,
     A                 LASTIN, MAGEL,  MHI,    MONQN,  MSET,
     B                 MUNIT,  MXCALC, NFVARY, NNRG,   NOBIS,
     C                 NODMAX, NODMIN, NUMDER, NWVCOL, PHILW,
     D                 PHIST,  POWRL,  POWRS,  POWRX,  PRNTLV,
     E                 RMATCH, RMAX,   RMID,   RMIN,   RUNAME,
     F                 RUNIT,  SCALAM, STEPL,  STEPS,  THETLW,
     G                 THETST, TOLHI,  TOLHIL, TOLHIS, URED,
     H                 WKBMN,  WKBMX,
C
...
C
   ...
   10 MX=MXSAVE
      ...
      CALL GTIME(CTIME)
C
C  SET PARAMETERS IN MEMORY MODULE
C
      NIPR=2
      IXNEXT=1
...
C  INITIALISE IPERTN TO 0 SO THAT HAMMAT DOES NOT ATTEMPT PERTURBATION
      IPERTN=0
      ISCRUR=0
      ITPSUB=0
      MAPEFV=0
...
C
      NNRG=MIN(NNRG,MXNRG)
      IF (NNRG.GT.1 .AND. DNRG.NE.0.D0) THEN
        DO I=2,NNRG
...
C  IF NEXTRA HAS BEEN SET, ASSUME THAT DEGTOL IS NEEDED
 ...
 1102   FORMAT(2X,'THERE ARE ',I3,' EXTRA OPERATORS')
      ENDIF
C
      IF (FLDMIN.EQ.0.D0 .AND. FLDMAX.EQ.0.D0) THEN
        WRITE(6,*) 'YOU HAVE NOT SPECIFIED A RANGE OF FIELD'
        STOP
      ENDIF

      IF (FLDMAX.EQ.FLDMIN) DFIELD=1.D0
 ...
 1109   FORMAT(/' RMIN MUST BE FIXED IF WAVEFUNCTIONS ARE BEING ',
     ...
     1                       (IPROPL.EQ.6 .OR. IPROPL.EQ.9))
      IF (IWAVE.NE.0 .AND. .NOT.WAVE) THEN
        WRITE(6,1110)
 1110   FORMAT(/'  WAVEFUNCTIONS ONLY AVAILABLE FOR LDMD AND AIRY '
     1          'PROPAGATORS: REQUEST CANCELLED')
      ENDIF
C
...
C
 ...
 2100     FORMAT(/2X,29('*'),'  ANGULAR MOMENTUM JTOT  =',I4,'  ',
     1           ' AND SYMMETRY BLOCK  = ',I4,2X,28('*'))
          IF (LSCRUR) THEN
            OPEN(ISCRUR,FORM='UNFORMATTED',STATUS='SCRATCH')
          ENDIF
...
C
     ...
     2                          EUNIT,EUNAME,QNAME,IBOUND,IPRINT,NNRG,
     3                          'ENERG'//YPLUR(MIN(NNRG,2)))
C
...
C
            IF (LFSCAN) GOTO 330
C
            NBDST=ABS(NODHI-NODLO)
            NBDST=MIN(NBDST,MXNODE)
            NBDBEG=MIN(NODLO,NODHI)
            NBDEND=MIN(NBDST+NBDBEG,MAX(NODLO,NODHI))
            IF (WAVE) CALL WVSETS(ENERGY(INRG),EUNIT,EUNAME,EREF,INRG,
     1                            NBDBEG+1,NBDEND)
...
C
              IF (FLDHI(IBDST)-FLDLO(IBDST).GT.100.D0*DTOL) THEN
                IF (UPWARD .AND.
     1              (EIGHI(IBDST).LT.0.D0 .OR. EIGLO(IBDST).GT.0.D0))
     2            GOTO 510
                IF (.NOT.UPWARD .AND.
     1              (EIGHI(IBDST).GT.0.D0 .OR. EIGLO(IBDST).LT.0.D0))
     2            GOTO 510
              ENDIF
C
...
C
              IF (EIGHI(IBDST)*EIGLO(IBDST).GE.0.D0) THEN
                IF (FLDHI(IBDST)-FLDLO(IBDST).GT.100.D0*DTOL) GOTO 510
