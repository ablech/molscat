...
C
      ...
      DIMENSION SIGCUR(*), SIGACC(*), SIGDEG(*), IXX(*),RXX(*)
...
C  AND CENT
 2000 IFAIL=0
     ...
     1           IFSET,DUM,DUM,DUM,DUM,DUM,DUM,IP,IBOUND,ITPSUB,1,IFAIL)
...
C   BUT THAT'S PROBABLY NOT AN ISSUE AT THIS POINT)
      IXNB=IXNEXT                           ! NB
      IXWV=IXNB+NOPMX                       ! WVEC
      IXSR=IXWV+NOPMX                       ! SR
      IXSI=IXSR+NOPMX*NOPMX                 ! SI
      IXL=IXSI+NOPMX*NOPMX                  ! L
      IXJIND=IXL+NOPMX                      ! JSINDX
      IXIND=IXJIND+MAX(NSTATE,NOPMX)        ! INDLEV
      IXCENT=IXIND+MAX(NSTATE,NOPMX)        ! CENT
C  NO SPACE NEEDED FOR CENT, EINT - ONLY USED IF IPRINT>10
      IXEINT=IXCENT !+NOPMX                 ! EINT
      IXNEXT=IXEINT !+NOPMX
      NUSED=0
      CALL CHKSTR(NUSED)
C  NEXT ASSIGN SPACE FOR TEMPORARIES FOR SPROC
      ITKMAT=IXNEXT                 ! KMAT } THESE ARRAYS WILL NOT BE
      ITWK1=ITKMAT  ! +NOPMX*NOPMX  ! WK1  } USED BECAUSE THE K MATRIX
      IXNEXT=ITWK1  ! +NOPMX               } IS NOT CALCULATED
...
C  TO 0 IN CALLS TO SPROC FROM RESTRT)
      IXIUSE=ITWK1  ! +NOPMX        ! INDUSE
      IXIACC=IXIUSE ! +NOPMX        ! INDACC
      IXLNEV=IXIACC ! +NSIG         ! LNEVER
      IXNEXT=IXLNEV ! +NSIG
      NUSED=0
      CALL CHKSTR(NUSED)
...
C
 ...
 4000 IFAIL=0
      JT=JT+1
      CALL SLPRD(ISAVEU,JTOT,INRG,ECHK,EFV,EREF,IEXCH,WGHT,M,NOPEN,
     1           IFSET,X(IXIJ),X(IXL),X(IXCENT),X(IXWV),
     2           X(IXSR),X(IXSI),
...
C  SPROC IS USED TO PROCESS S MATRICES INTO CROSS SECTIONS.
      ...
      CALL SPROC(JTOT,X(IXNB),X(IXJIND),X(IXL),X(IXIND),
     1           X(IXWV),X(IXSR),X(IXSI),X(ITKMAT),RUNIT,
     2           SCLEN,ESUM,NOPEN,M,MXPAR,WGHT,IEXCH,
     3           INRG,TTIME,ENERGX(INRG),EREF,SIGCUR,SIGACC(IACC),
     4           SIGDEG,JSTATE,ISST,ICAC,
     5           MINJT(JT),MAXJT(JT),NSTATE,
     6           NQN,OTOL,DTOL,0,ISIGU,IPARTU,ISAVEU,
     7           ISIGPR,IRSTRT,ICHAN,X(ITWK1),X(IXCENT),
     8           X(IXEINT),IBOUND,X(IXIUSE),X(IXIACC),
     9           X(IXLNEV),CM2RU,IFSET,JPRINT,.TRUE.,.FALSE.,
     A           AWVMAX,RUNAME)
C
...
C  CODE FOR ORDER STARTING WITH VERSION 13X ...
     ...
     1          X(IXNB),X(IXJIND),X(IXL),X(IXWV),
     2          X(IXSR),X(IXSI),X(IT1),X(IT2),X(IT3),X(IT4),
