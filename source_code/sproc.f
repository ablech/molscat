...
C  Distributed under the GNU General Public License, version 3
     ...
     e                LEFVN, LEFVU, LISTFV, NEFV, NEFVP, NNZRO
      USE potential, ONLY: NCONST, NDGVL, NRSQ
...
C
C  ENTRY SPROC  - PROCESSES S MATRICES TO X-SECTIONS OUTPUTS THEM.
C  ENTRY SIGSAV - (10/92) SEPARATE ENTRY TO UPDATE UNIT(ISIGU)
C                 TO FIX BUG W/ NO BASIS FNS FOR SYMMETRY BLOCK IB=IBMAX
C  ENTRY SPINIT - INITIALIZATION ENTRY FROM DRIVER.
C  ENTRY SIGWRT - PUNCHES FINAL CROSS SECTIONS.
...
C            0 AS A FLAG THAT THERE WAS NO SPACE FOR THE LEVEL
            IF (INDLEV(NB).EQ.-1) THEN
              WRITE(6,*) ' SPROC ERROR: CHANNEL ',I,' WAS FLAGGED'//
...
C  WHERE M = MIN(LRPOW-2,2*LL+1).
C  LRPOW WOULD NEED TO BE CHANGED IF THE LONG-RANGE POWER IS NOT -6
C
...
C
  ...
  780   FORMAT('  *** SPINIT.  RESTART FROM UNIT(',I3,').  IRSTRT =',I4)
        WRITE(6,*) ' *** '
        IF (IPHSUM.NE.0) THEN
...
C  THESE FORMAT STATEMENTS ARE USED IN SIGSAV FOR SUMMARY WRITTEN TO STDOUT
      F302="(/'  SIGSAV: DA FILE (',I2,') UPDATED WITH SIGMA "//
